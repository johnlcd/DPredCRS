...
class ML:
	...
	def naive_bayes_classifier(self, gscv=False):
	...
	def knn_classifier(self, gscv=False):
		...
		if (gscv!=False):
			model = GridSearchCV(KNeighborsClassifier(), param_grid, cv=5, n_jobs=3, scoring=self.scorer, verbose=1)
		else:
			model = KNeighborsClassifier()
		return model
	 
	# Logistic Regression Classifier (RFE, train)
	def logistic_regression_classifier(self, gscv=False):
		...
		if (gscv!=False):
			model = GridSearchCV(LogisticRegression(),param_grid, cv=5, n_jobs=3, scoring=self.scorer, verbose=1)
		else:
			model = LogisticRegression(penalty='l2',max_iter=1000)
		return model
	 
	# Random Forest Classifier (RFE,train)
	def random_forest_classifier(self, gscv=False):
		from sklearn.ensemble import RandomForestClassifier
		param_grid = {
		...
		if (gscv!=False):
			model = GridSearchCV(RandomForestClassifier(class_weight="balanced", random_state=self.seed), param_grid, cv=5, n_jobs=3, scoring=self.scorer, verbose=1)
		else:
			model = RandomForestClassifier(n_estimators = self.nest, max_depth = self.mdepth, class_weight="balanced", random_state=self.seed) #RFECV
		return model
	 
	 
	# Decision Tree Classifier (train)
	def decision_tree_classifier(self, gscv=False):
		from sklearn import tree
		model = tree.DecisionTreeClassifier()
		return model
	 
	# GBDT(Gradient Boosting Decision Tree) Classifier (train)
	def gradient_boosting_classifier(self, gscv=False):
		...
		if (gscv!=False):
			model = GridSearchCV(GradientBoostingClassifier(random_state=self.seed), param_grid, cv=5, n_jobs=3, scoring=self.scorer, verbose=1)
		else:
			model = GradientBoostingClassifier(n_estimators=100,random_state=self.seed)
		return model
	 
	 
	# SVM Classifier
	def svm_classifier(self, gscv=False):
	...
	def lsvm_classifier(self, gscv=False):
		...
		return model
	## "RBF" kernel
	def svm_rbf_classifier(self, gscv=False):
		from sklearn.svm import SVC
		param_grid = {
			'C': [0.1, 1, 10],
			'gamma': [0.1, 0.01, 'auto'],
		...
		return model
	## "Poly" kernel
	def svm_poly_classifier(self, gscv=False):
		from sklearn.svm import SVC
		param_grid = {
			'C': [0.1, 1, 10],
			'degree': [1, 3, 5]
		}
		model = GridSearchCV(SVC(kernel='poly', random_state=self.seed, probability=True), param_grid, cv=5, n_jobs=6, scoring=self.scorer, verbose=1)
	...
	def rfe_cv(self, data):
		...
		if self.func == "rfe":
			## RFE
			model_rfe = RFE(clf,step=STEP,n_features_to_select=self.nf_select,verbose=1)
			...
			NUM_PARAL = NUM_CV
			model_rfe = RFECV(estimator=clf,step=STEP,cv=cv,min_features_to_select=self.min_fs,scoring=self.scorer,verbose=1,n_jobs=NUM_PARAL) ## "roc_auc"
		...
		if self.model_save_file != None:
			...
			open_file = open(fs_model_save, 'wb')
	...
	def fit(self, data0, data1, data2, model_rfe0, model_rfe1, model_rfe2, fs0, fs1, fs2):
		...
		model_t0 = classifiers[self.classifier](gscv=True)
		model_t1 = classifiers[self.classifier](gscv=True)
		model_t2 = classifiers[self.classifier](gscv=True)
		print('\n===============================  Group 0  ================================')
		model_t0.fit(train_x0_t,train_y0) if self.classifier == "LR" else self.pch_sklearn(model_t0,train_x0_t,train_y0)
		gb_best_m0 = model_t0.best_estimator_
		...
		print("### Estimator: ",gb_best_m0)
		print('### Best Parameters: ',model_t0.best_params_)
		print('\n>>>>> Model evaluation <<<<<')
		...
		gb_best_m1 = model_t1.best_estimator_
		...
		print(">>>>> Estimator && Parameters <<<<<")
		print("### Estimator: ",gb_best_m1)
		print('### Best Parameters: ',model_t1.best_params_)
		print('\n>>>>> Model evaluation <<<<<')
		print('--   Dataset    --||-- F1 score  --||----  AUC  ----||----  Acc  ----||----  Pre  ----||----  Rec  ----||--  N Sample  --')
		...
		gb_best_m2 = model_t2.best_estimator_
		...
		print(">>>>> Estimator && Parameters <<<<<")
		print("### Estimator: ",gb_best_m2)
		print('### Best Parameters: ',model_t2.best_params_)
		print('\n>>>>> Model evaluation <<<<<')
		print('--   Dataset    --||-- F1 score  --||----  AUC  ----||----  Acc  ----||----  Pre  ----||----  Rec  ----||--  N Sample  --')
