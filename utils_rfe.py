...
def make_result_dir(path):
	"""
	Creat directories for results.
	"""
	if (not os.path.exists(path)):
		os.system("mkdir -p " + path)
	fs_path = path + "/fs_res"
	score_path = path + "/rfecv_score"
	model_path = path + "/model_save"
	fs_model_path = path + "/model_save/fs"
	perf_path = path + "/model_perf"
	list_dir = [fs_path, fs_path+"/fea_rank", fs_path+"/fea_sel", score_path, model_path, fs_model_path, perf_path]
	os.system("mkdir -p " + " ".join(list_dir))
...
def scale_norm_fea(mx, with_mean = True, with_std = True, norm = 'l2'):
	...
	#print("### Mean of scaler:\n   ",scaler.mean_)
	#print("### Scale of scaler:\n   ",scaler.scale_)
...
def load_data_gp(path, dataset, rapt, group, seed):
	...
	feature_dict = {}
	label_dict = {}
	file_fea_label = "{}/RAPT_{}/{}_G{}_{}".format(path, rapt, dataset, group, 'fea_label')
